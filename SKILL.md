...
name: mcp-sc
description: >
  SuperCollider ワークフロー用 MCP サーバー (Python)。music-json を読み、
  SuperCollider NRT (recordNRT) で WAV レンダリングする。MCP (JSON-RPC 2.0 over stdio)
  として opencode / Claude 等から music_render / music_generate 等を呼べる。
  「supercollider」「sc」「music_render」「music_generate」「NRT」
  「music-json」「音楽レンダリング」「mcp-sc」「scsynth」などのキーワードで発動。
  実行: `PYTHONPATH=. python3 lib/mcp_sc.py`
---

# mcp-sc — SuperCollider MCP サーバー (Python)

music-json → SuperCollider NRT → WAV レンダリング。
